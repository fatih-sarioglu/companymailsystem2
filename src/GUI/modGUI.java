...
public class modGUI extends javax.swing.JFrame {

    DbConnection conn = new DbConnection();
    static Moderator mod = new Moderator();
    private DefaultTableModel reqModel = null;  
    private Object[] reqData = null;
    ...
    private void initComponents() {
        ...
        colReqName[0] = "Name";
        colReqName[1] = "Email";
        colReqName[2] = "Password";
        reqModel.setColumnIdentifiers(colReqName);
        reqData = new Object[3];
        try{
            for (int i = 0; i < mod.getReqList().size(); i++){
                reqData[0] = mod.getReqList().get(i).getName();
                reqData[1] = mod.getReqList().get(i).getEmail();
                reqData[2] = mod.getReqList().get(i).getPassword();
                reqModel.addRow(reqData);
            }
        }catch(Exception e){
        ...
        try{
            for (int i = 0; i < mod.getEmpList().size(); i++){
                empData[0] = mod.getEmpList().get(i).getId();
                empData[1] = mod.getEmpList().get(i).getName();
                empData[2] = mod.getEmpList().get(i).getAddress();
                empData[3] = mod.getEmpList().get(i).getPassword();
                empData[4] = mod.getEmpList().get(i).getType();
        ...
        Object[] colReq1Name = new Object[3];
        colReq1Name[0] = "Date";
        colReq1Name[1] = "From";
        colReq1Name[2] = "Subject";

        Object[] colReq2Name2 = new Object[3];
        colReq2Name2[0] = "Date";
        colReq2Name2[1] = "To";
        colReq2Name2[2] = "Subject";

        sentModel.setColumnIdentifiers(colReq2Name2);
        sentData = new Object[3];

        inboxModel.setColumnIdentifiers(colReq1Name);
        inboxData = new Object[3];
        spamModel.setColumnIdentifiers(colReq1Name);
        spamData = new Object[3];
        archModel.setColumnIdentifiers(colReq1Name);
        archData = new Object[3];
        trashModel.setColumnIdentifiers(colReq1Name);
        trashData = new Object[3];
        try{

            for (int i = 0; i < mod.getMailList(mod.getId()).size(); i++){
                switch(mod.getMailList(mod.getId()).get(i).getMailBox()){
                    case "1":
                    inboxData[0] = mod.getMailList(mod.getId()).get(i).getMailTime();
                    inboxData[1] = mod.getMailList(mod.getId()).get(i).getMailFrom();
                    inboxData[2] = mod.getMailList(mod.getId()).get(i).getMailSubject();
                    inboxModel.addRow(inboxData);
                    break;
                    case "2":
                    spamData[0] = mod.getMailList(mod.getId()).get(i).getMailTime();
                    spamData[1] = mod.getMailList(mod.getId()).get(i).getMailFrom();
                    spamData[2] = mod.getMailList(mod.getId()).get(i).getMailSubject();
                    spamModel.addRow(spamData);
                    break;
                    case "3":
                    archData[0] = mod.getMailList(mod.getId()).get(i).getMailTime();
                    archData[1] = mod.getMailList(mod.getId()).get(i).getMailFrom();
                    archData[2] = mod.getMailList(mod.getId()).get(i).getMailSubject();
                    archModel.addRow(archData);
                    break;
                    case "4":
                    trashData[0] = mod.getMailList(mod.getId()).get(i).getMailTime();
                    trashData[1] = mod.getMailList(mod.getId()).get(i).getMailFrom();
                    trashData[2] = mod.getMailList(mod.getId()).get(i).getMailSubject();
                    trashModel.addRow(trashData);
                    break;
                    case "5":
                    sentData[0] = mod.getMailList(mod.getId()).get(i).getMailTime();
                    sentData[1] = mod.getMailList(mod.getId()).get(i).getMailTo();
                    sentData[2] = mod.getMailList(mod.getId()).get(i).getMailSubject();
        ...
        workerReq = new javax.swing.JScrollPane();
        reqTable = new javax.swing.JTable(reqModel);
        employees = new javax.swing.JScrollPane();
        ...
        byte[] bytes = mod.getName().getBytes(StandardCharsets.UTF_8);
        String utf8EncodedName = new String(bytes, StandardCharsets.UTF_8);
        welcomeText.setText("Welcome, " + utf8EncodedName);
        contentPane.add(welcomeText, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 20, 340, 30));
        ...
        spamItem.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e10){
                try{
                    ...
                    String query = String.format("UPDATE users_mails.%dmails SET mail_box = '2' WHERE mail_subject ='" + selSub + "'",mod.getId());
        ...
        archItem.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e10){
                try{
                    ...
                    Statement st = con.createStatement();
                    inboxModel.removeRow(inboxTable.getSelectedRow());
                    String query = String.format("UPDATE users_mails.%dmails SET mail_box = '3' WHERE mail_subject ='" + selSub + "'",mod.getId());
                    st.executeUpdate(query);
                } catch(SQLException e2){
                    e2.printStackTrace();
                } catch(Exception e){
        ...
        trashItem.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e10){
                try{
                    ...
                    Statement st = con.createStatement();
                    inboxModel.removeRow(inboxTable.getSelectedRow());
                    String query = String.format("UPDATE users_mails.%dmails SET mail_box = '4' WHERE mail_subject ='" + selSub + "'",mod.getId());
                    st.executeUpdate(query);
                } catch(SQLException e2){
                    e2.printStackTrace();
                } catch(Exception e){
        ...
        openItem.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e10){
                try{
                    ...
                    Connection con = conn.connDb();
                    Statement st = con.createStatement();
                    String query = String.format("SELECT mail_content FROM users_mails.%dmails WHERE mail_subject ='" + selSub + "'",mod.getId());
        ...
        sInboxItem.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e10){
                try{
                    ...
                    String query = String.format("UPDATE users_mails.%dmails SET mail_box = '1' WHERE mail_subject ='" + selSub + "'",mod.getId());
                    st.executeUpdate(query);
                } catch(SQLException e2){
                    e2.printStackTrace();
                } catch(Exception e){
        ...
        tDeletePerm.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e10){
                try{
                    ...
                    Statement st = con.createStatement();
                    trashModel.removeRow(trashTable.getSelectedRow());
                    String query = String.format("DELETE FROM users_mails.%dmails WHERE mail_subject ='" + selSub + "'",mod.getId());
                    st.executeUpdate(query);
                } catch(SQLException e2){
                    e2.printStackTrace();
                } catch(Exception e){
        ...
        reqTable.setFocusable(false);
        reqTable.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mousePressed(java.awt.event.MouseEvent evt) {
                reqTableMousePressed(evt);
            }
        });
        workerReq.setViewportView(reqTable);
        reqMenu = new JPopupMenu();
        JMenuItem deleteReq = new JMenuItem("Delete Request");
        JMenuItem acceptReq = new JMenuItem("Accept Request");
        reqMenu.add(deleteReq);
        reqMenu.add(acceptReq);
        reqTable.setComponentPopupMenu(reqMenu);

        acceptReq.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e10){
                try{
                    String selEmail = reqTable.getValueAt(reqTable.getSelectedRow(), 1).toString();
                    String selName = reqTable.getValueAt(reqTable.getSelectedRow(), 0).toString();
                    String selPassword = reqTable.getValueAt(reqTable.getSelectedRow(), 2).toString();
                    Connection con = conn.connDb();
                    Statement st = con.createStatement();
                    reqModel.removeRow(reqTable.getSelectedRow());
                    String query = "INSERT INTO users.users_table (users_name,users_email,users_password,users_type) VALUES ('" + selName + "', '" + selEmail + "', '" + selPassword + "', 'WORKER' )";
                    st.executeUpdate(query);
                    ResultSet rs = st.executeQuery("SELECT * FROM users.users_table");
                    int count = 0;
                    while(rs.next()){
                        count = rs.getInt("id_users_table");
                    }
                    String query3 = String.format("""
                        CREATE TABLE `users_mails`.`%dmails` (
                            ...
                            `mail_box` VARCHAR(45) NOT NULL)
                        ENGINE = InnoDB
                        DEFAULT CHARACTER SET = utf8;""", count);
                        st.executeUpdate(query3);
                        String query4 = "DELETE FROM user_reqs.reqs WHERE req_email = '" + selEmail + "'";
                        st.executeUpdate(query4);
                    } catch(SQLException e2){
                        Logger.getLogger(LoginPage.class.getName()).log(Level.SEVERE, null, e2);
                    } catch(Exception e){
                        Helper.Helper.showMsg("choose");
                    }
            ...
            deleteReq.addActionListener(new ActionListener(){
                public void actionPerformed(ActionEvent e10){
                    try{
                        ...
                        Statement st = con.createStatement();
                        reqModel.removeRow(reqTable.getSelectedRow());
                        String query = "DELETE FROM user_reqs.reqs WHERE req_email = '" + selEmail + "'";
                        st.executeUpdate(query);
            ...
            reqMenu.addMouseListener(new MouseAdapter(){
                public void mousePressed(MouseEvent mevt){
                    Point point = mevt.getPoint();
                    int selectedRow = reqTable.rowAtPoint(point);
                    reqTable.setRowSelectionInterval(selectedRow, selectedRow);
                }
            });

            tabs.addTab("Worker Requests", workerReq);

            empTable.setFocusable(false);
            empTable.setModel(empTableModel);
            employees.setViewportView(empTable);
        ...
        sendButton.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ...
                try{
                    ...
                    String query3 = String.format("INSERT INTO users_mails.%smails (mail_to,mail_from,mail_subject,mail_content,mail_time,mail_box) VALUES (?,?,?,?,?,?)", mod.getId());
                    PreparedStatement ps2 = con.prepareStatement(query3);
                    ps2.setString(1, toMail);
                    ps2.setString(2, mod.getAddress());
    ...
    */
    ...
    contentPane.add(tabs, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 60, 480, 520));
    ...
    private void reqTableMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_reqTableMousePressed
            
    }//GEN-LAST:event_reqTableMousePressed
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(modGUI.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            @Override
            public void run() {
                new modGUI(mod).setVisible(true);
            }
        });
    }
    ...
    private javax.swing.JScrollPane employees;
    ...
    private javax.swing.JPopupMenu reqMenu;
    ...
    private javax.swing.JScrollPane workerReq;
